...
class Router {
    ...
    async loadPageData(pageName) {
        try {
            switch (pageName) {
                case 'home':
                    if (window.CategoryRenderer) CategoryRenderer.loadCategories();
                    if (window.ProductRenderer) ProductRenderer.loadFeaturedProducts();
                    break;
                    
                case 'products':
                    if (window.ProductRenderer) ProductRenderer.loadProducts();
                    break;
                    
                case 'cart':
                    if (window.CartRenderer) CartRenderer.loadCart();
                    break;
                    
                case 'orders':
                    if (window.OrderRenderer) OrderRenderer.loadOrders();
                    break;
                    
                case 'profile':
                    if (window.ProfileRenderer) ProfileRenderer.renderProfile();
                    break;
            }
        } catch (error) {
